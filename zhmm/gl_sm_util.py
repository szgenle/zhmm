...
from zhmm.utils import array_ex
...
def hash_by_sm3(input_bytes, key ='9gx^1-z:ixYWe(@JAJKFu1*k@913^ka1'):
    ...
    key_hash = sm3.sm3_hash(array_ex.string_to_hex_array(key))
    key_array = array_ex.hex_to_array(key_hash)
    ...
    input_bytes = opad_key + array_ex.hex_to_array(hash_)
    return sm3.sm3_hash(input_bytes)


def decrypt_by_sm4(encrypt_value, key):
    encrypt_value = array_ex.hex_to_array(encrypt_value)
    crypt_sm4 = sm4.CryptSM4()
    key = array_ex.hex_to_array(key)
